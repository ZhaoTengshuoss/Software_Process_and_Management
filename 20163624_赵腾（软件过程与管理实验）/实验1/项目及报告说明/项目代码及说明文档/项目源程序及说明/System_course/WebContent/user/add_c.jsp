...
<div style="margin-left:50px;margin-right:30px;margin-top:0px;font-size:50px">
	...
	<table  class="layui-table" style="text-align:center">
		...
		<tr>
		   ...
		   <td>
		   <input name="Tno" id="Tno" onfocus="TnoGetFocus()" onblur="TnoLoseFocus()"/>
		    <span id="Tno_span"></span><br/>
		   </td>
		</tr>
		   <td>
		   课程学分：
...
List<User> users_t=userDao.find_t();
int j=0;
for(User user_t :users_t)
{
	j++;
	%>
	<input type="hidden" name="users_t" value="<%=user_t.getTno()%>"></input>
	<%
	}
	%>
	<input type="hidden" id="length_t" name="length_t" value="<%=j%>"></input>
<%
int i=0;
for(User user :users)
{
...
var le2=document.getElementById("length_t").value;
var no2= new Array(le2);
no2=document.getElementsByName("users_t");
...
var spanId=document.getElementById("Cno_span");
spanId.innerHTML="<font color='blue' size='2'>您输入的课程编号格式正确！</font>";
return true;
     }
...
function TnoGetFocus(){
	//获取userspan的Id
	var spanId=document.getElementById("Tno_span");
	//给span中添加内容
	spanId.innerHTML="<font color='blue' size='2'>请输入老师工号</font>"
	}
	//使用用户失去焦点的函数
	function TnoLoseFocus(){
	//这里先获取input文本框 的内容
    var flag2=false; 
	...
	if(len==0){
	var spanId=document.getElementById("Tno_span");
	spanId.innerHTML="<font color='red' size='2'>*老师工号不能为空</font>";
	return false;
	}else if(!flag2)
	{
		var spanId=document.getElementById("Tno_span");
		spanId.innerHTML="<font color='red' size='2'>您输入的老师工号不存在！</font>";
		flag2=false;
		return false;
	...
	var spanId=document.getElementById("Tno_span");
	spanId.innerHTML="<font color='blue' size='2'>您输入的老师工号格式正确！</font>";
	return true;
	     }
...
var Tno_result=TnoLoseFocus();
if(Cno_result&& Cname_result&&type_result&&Ccredit_result&&Semester_result&&TnoLoseFocus()){
return true;
}else{
return false;
} 
