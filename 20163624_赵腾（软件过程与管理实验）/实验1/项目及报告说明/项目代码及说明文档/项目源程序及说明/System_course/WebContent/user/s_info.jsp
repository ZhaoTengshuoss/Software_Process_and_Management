...
<%
   ...
   String content = request.getParameter("content");
	...
	UserDaoImpl userDao = new UserDaoImpl();
	 List<User> users = userDao.load1(content); 
	
	
%>
<form action="s_info.jsp"  method="get">
 <div class="my-btn-box" style="margin:10px 10px 10px 10px">
    <!--<span style="float:left;">
        <a class="layui-btn btn-add btn-default" id="btn-add" href="add_s.jsp">添加新用户</a> 
    </span> -->
    <span style="float:right">
        <span class="layui-form-label">搜索条件：</span>
        <div class="layui-input-inline">
            <input type="text" autocomplete="off" placeholder="请输入学生姓名或学号" class="layui-input" name="content">
        </div>
 ...
 <div style="margin-left:50px;margin-right:30px;margin-top:100px;font-size:50px"> 
       <table class="layui-table">
			...
			<th style="font-size:30px;text-align:center;">学生性别</th>
			<th style="font-size:30px;text-align:center;">学生年龄</th>
		...
		<tr>
			...
			<td style="font-size:30px;text-align:center;"><%=user.getSsex() %></td>
			<td style="font-size:30px;text-align:center;"><%=user.getSage() %></td>
			<td style="font-size:30px;text-align:center;"><%=user.getSdept() %></td>
			<td> <a href="delete.jsp?s_id=<%=user.getSno() %>"><span style="color:blue;font-size:30px">删除</span></a></td>
			<td> <a href="updateInput.jsp?s_id=<%=user.getSno() %>"><span style="color:blue;font-size:30px">修改</span></a></td>
			<td> <a href="show.jsp?s_id=<%=user.getSno() %>"><span style="color:blue;font-size:30px">查看课程信息</span></a></td>
