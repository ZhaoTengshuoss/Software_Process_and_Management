...
{
	id2=Integer.parseInt(request.getParameter("s_id"));
	}

    UserDaoImpl userDao=DaoFactory.getDaoImpl();
    
    User user=userDao.find_user(id2);      
...
<form class="layui-form" action="update.jsp" onsubmit="return tijiao()">
<table  class="layui-table">
&nbsp; <span style="color:blue; font-family:楷体 ;font-size:20px"><%=Meg %></span>
<input type="hidden" name="id" value="<%=user.getSno()%>"/>
  <input type="hidden" name="ssex" value="<%=user.getSsex()%>"/>
  <tr><td>名字:</td>
  <td> <input type="text" name="姓名" value="<%=user.getSname() %>"onfocus="nameGetFocus()" onblur="nameLoseFocus()"><span id="name_span"></span><br/>    </td>
  </tr>
  <tr><td>年龄:</td>
  <td> <input type="text" name="年龄" value="<%=user.getSage() %>"onfocus="ageGetFocus()" onblur="ageLoseFocus()"><span id="age_span"></span><br/></td>
  </tr>
  <tr><td>所在系:</td>
  <td> <input type="text" name="所在系" value="<%=user.getSdept() %>"onfocus="deptGetFocus()" onblur="deptLoseFocus()"> <span id="dept_span"></span><br/></td>
  </tr>
</table>
    <br>
